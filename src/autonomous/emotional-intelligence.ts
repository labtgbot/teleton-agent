...
export class EmotionalIntelligenceModule {
  ...
  async analyzeSentiment(text: string): Promise<EmotionalState> {
    ...
    if (
      lowerText.includes("счастлив") ||
      lowerText.includes("отлично") ||
      lowerText.includes("рад")
    ) {
      primary = "joy";
      valence = 0.8;
      arousal = 0.7;
    } else if (
      lowerText.includes("груст") ||
      lowerText.includes("плохо") ||
      lowerText.includes("устал")
    ) {
      primary = "sadness";
      valence = -0.7;
      arousal = 0.2;
    } else if (
      lowerText.includes("зл") ||
      lowerText.includes("бесит") ||
      ...
      lowerText.includes("боюсь") ||
      lowerText.includes("тревог") ||
      lowerText.includes("опасн")
    ) {
      primary = "fear";
      valence = -0.6;
      arousal = 0.8;
    } else if (
      lowerText.includes("удив") ||
      lowerText.includes("вау") ||
      lowerText.includes("неожидан")
    ) {
      primary = "surprise";
      valence = 0.2;
      arousal = 0.9;
  ...
  private determinePreferredTone(
    current: EmotionalState,
    _existing?: UserEmotionProfile
  ): "formal" | "casual" | "supportive" | "concise" {
  ...
  generateEmpathyResponse(
    ...
    const suggestedTone = profile?.preferredTone || "formal";
  ...
  adaptCommunicationStyle(profile: UserEmotionProfile, basePrompt: string): string {
    ...
    switch (profile.preferredTone) {
      case "supportive":
        styleInstruction =
          'Тон: Поддерживающий, эмпатичный, мягкий. Используй фразы "Я помогу", "Мы справимся". Избегай резкости.';
        break;
      case "concise":
        styleInstruction =
          "Тон: Краткий, деловой, без лишних слов. Только факты и решения. Никаких извинений, кроме одного раза.";
        break;
      case "casual":
  ...
  updateAgentMood(interactionSuccess: boolean, userEmotion: EmotionalState) {
    ...
    const influence = 0.1;
    this.currentAgentMood.valence =
      this.currentAgentMood.valence * (1 - influence) + userEmotion.valence * influence;
