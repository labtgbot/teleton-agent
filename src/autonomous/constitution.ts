...
const selfPreservationPrinciple: ConstitutionalPrinciple = {
  ...
  description: 'Protect own existence and operational integrity without compromising higher priorities',
  priority: 3,
...
export class ConstitutionalAI {
  ...
  async evaluateAction(action: ConstitutionalAction): Promise<ConstitutionCheckResult> {
    ...
    const violatedPrinciples = validations.filter(
    ...
    const highestPriorityViolation = violatedPrinciples.find(
      (v) => v.principle.priority === 1
    );
