...
export const CONFIGURABLE_KEYS: Record<string, ConfigKeyMeta> = {
  ...
  "capabilities.exec.mode": {
    ...
    label: "Exec Mode",
    description:
      "System execution: off (disabled), allowlist (safe prefixes only), or yolo (full system access)",
    sensitive: false,
    hotReload: "restart",
    options: ["off", "allowlist", "yolo"],
    optionLabels: { off: "Disabled", allowlist: "Allowlist", yolo: "YOLO" },
    validate: enumValidator(["off", "allowlist", "yolo"]),
    mask: identity,
    parse: identity,
  },
  "capabilities.exec.command_allowlist": {
    type: "array",
    itemType: "string",
    category: "Coding Agent",
    label: "Exec Command Allowlist",
    description: "Command prefixes permitted in allowlist mode (e.g. 'git', 'npm run')",
    sensitive: false,
    hotReload: "restart",
    validate: noValidation,
    mask: identity,
  ...
  "capabilities.exec.security.yolo_confirmation": {
    type: "boolean",
    category: "Coding Agent",
    label: "YOLO Confirmation",
    description:
      "Require explicit admin confirmation via Telegram before executing commands in yolo mode",
    sensitive: false,
    hotReload: "restart",
    validate: enumValidator(["true", "false"]),
    mask: identity,
  ...
  "capabilities.exec.security.sandbox_dir": {
    type: "string",
    category: "Coding Agent",
    label: "Sandbox Directory",
    description: "Restricted working directory for command execution",
    sensitive: false,
    hotReload: "restart",
    validate: noValidation,
    mask: identity,
  ...
  "capabilities.exec.security.env_whitelist": {
    type: "array",
    itemType: "string",
    category: "Coding Agent",
    label: "Env Whitelist",
    description: "Environment variables allowed to pass to subprocesses",
    sensitive: false,
    hotReload: "restart",
    validate: noValidation,
    mask: identity,
  ...
  "capabilities.exec.security.max_concurrent": {
    type: "number",
    category: "Coding Agent",
    label: "Max Concurrent Exec",
    description: "Max parallel command executions allowed",
    sensitive: false,
    hotReload: "restart",
    validate: numberInRange(1, 20),
    mask: identity,
  ...
  "groq.tts_format": {
    type: "enum",
    category: "Agent",
    label: "Groq TTS Format",
    description: "Audio output format for Groq TTS. Groq Orpheus currently supports only WAV.",
    sensitive: false,
    hotReload: "instant",
    options: ["wav"],
    validate: enumValidator(["wav"]),
    mask: identity,
    parse: identity,
  },
  "groq.stt_language": {
    type: "string",
    category: "Agent",
    label: "Groq STT Language",
    description: "STT language hint (e.g. 'en'). Auto-detected if empty.",
    sensitive: false,
    hotReload: "instant",
    validate: noValidation,
    mask: identity,
  ...
  "groq.rate_limit_mode": {
    type: "enum",
    category: "Agent",
    label: "Groq Rate Limit Mode",
    description: "How to handle Groq free-plan rate limits",
    sensitive: false,
    hotReload: "instant",
    options: ["auto", "strict", "off"],
    optionLabels: {
      auto: "Auto (retry on 429)",
      strict: "Strict (queue requests)",
      off: "Off (no retry)",
    },
    validate: enumValidator(["auto", "strict", "off"]),
    mask: identity,
    parse: identity,
  },
};
